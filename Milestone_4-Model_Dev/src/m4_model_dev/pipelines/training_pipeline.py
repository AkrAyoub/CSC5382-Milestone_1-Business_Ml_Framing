...
def run_training_pipeline(config_path: Path | None = None) -> dict[str, Any]:
    ensure_runtime_dirs()
    config_path = config_path or (M4_CONFIGS_DIR / "train_best_model.yaml")
    ...
    # Fit on train only, then reuse the held-out validation split for threshold
    # selection before reporting test metrics.
    ...
    model_artifact_path = save_model_bundle(bundle, feature_columns, best_threshold, M4_ARTIFACTS_DIR)
    ...
    feature_importance_rows = extract_feature_importance(bundle, feature_columns)
    ...
    run_manifest = {
        ...
        "merged_dataset_path": str(M4_MERGED_DIR / "facility_training_dataset_with_splits.csv"),
    ...
    # Keep the MLflow upload set small and stable on the local file store.
    ...
    return {
        ...
        "feature_importance_path": feature_importance_path if feature_importance_rows else None,
        "emissions_path": emissions_path if emissions_path.exists() else None,
        "model_artifact_path": model_artifact_path,
        "mlflow_logged": bool(mlflow_result["mlflow_logged"]),
        "registered_model_name": mlflow_result.get("registered_model_name"),
